...
class App:
    def __init__(self):
        pyxel.init(256, 128, title="03 Dart ne marche pas")
    ...
    def update(self):        
        ...
        for singe in self.liste_singes:
            for ballon in self.liste_ballons:
                if fonctions_aux.distance((singe.x, singe.y),(ballon.x, ballon.y)) <= singe.range**2+singe.range:
    ...
    def draw(self):        
        ...
        for dart in self.liste_darts:
            pyxel.rect(dart.x, dart.y, 1, 1, 2)
